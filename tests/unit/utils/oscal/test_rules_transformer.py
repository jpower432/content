...
def test_rule_objs() -> List[RuleInfo]:
    ...
    test_parameter.set_default_value("default")
...
def test_rule_transformer_load(env_yaml: Dict[str, Any]):
    ...
    rule_objs = transformer.load(["rule_1", "rule_2"])

    assert len(rule_objs) == 2
    assert rule_objs[0].id == "rule_1"
    assert rule_objs[0].description == "This is a rule with a template example. It has two lines."
    ...
    assert rule_objs[0].parameters[0].default_value == "default"
...
def test_rule_transformer_load_with_param(env_yaml: Dict[str, Any]):
    ...
    rule_objs = transformer.load(["rule_1", "rule_2", "var_test=alternate"])

    assert len(rule_objs) == 2
    assert rule_objs[0].id == "rule_1"
    assert rule_objs[0].parameters is not None
    assert rule_objs[0].parameters[0].id == "var_test"
    assert rule_objs[0].parameters[0].default_value == "alternate"
    assert rule_objs[1].id == "rule_2"
    assert not rule_objs[1].parameters
...
def test_rules_transformer_transform(
    ...
    set2_props: List[Property] = next(
        (rulesets[ruleset] for ruleset in rulesets.keys() if ruleset == "rule_set_2"),
        [],
    )
